...
\section{Sparsity}
    ...
    \end{equation} where $n \in \mathbb{N}$ is the number of coefficients. Although many functions preserve this mapping, it needs to follow several criteria to be considered a sparsity measure. The one that preserves most of these criteria and provides a way to measure the sparsity of a distribution is the Gini Index \cite{10.2307/2223525}, being originally proposed in economics to measure the inequality of wealth but was later demonstrated to be used as a sparsity measure \cite{2006SparseSources, 2004GiniIndexSpeech}. It is defined as in Equation \ref{eq:gini_index}
     
    \begin{equation}
        \label{eq:gini_index}
        S(\textbf{v}) = 1 - 2\sum_{k=1}^{|\textbf{v}|} \frac{v_k}{||\textbf{v}||_1} \bigg( \frac{|\textbf{v}|-k+0.5}{|\textbf{v}|} \bigg)
    \end{equation} where $\textbf{v}$ is a vector with its $v_k$ coefficients sorted by magnitude from smallest to largest \cite{10.1109/TIT.2009.2027527}.
